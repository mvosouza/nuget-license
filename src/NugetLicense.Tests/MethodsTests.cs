...
namespace NugetLicense.Toolkit.Tests
{
    ...
    public class MethodTests
    {
        ...
        {
            _methods = new Methods(new CommandPackageOptions { ProjectDirectory = _projectPath, Timeout = 10 });
        }

        private void AddUniquePackageOption()
        {
            _methods = new Methods(new CommandPackageOptions { UniqueOnly = true, ProjectDirectory = _projectPath });
        }
...
#pragma warning restore CS1998 // Async method lacks 'await' operators and will run synchronously
        ...
        {
            var methods = new Methods(new CommandPackageOptions
            {
            ...
            result.Should()
                .HaveCount(1)
                .And.Match(kvp => kvp.First().Key.EndsWith("NugetLicense.Toolkit.csproj"));
        }
        ...
        public async Task GetProjectReferencesFromAssetsFile_Should_Resolve_Transitive_Assets()
        {
            var methods = new Methods(new CommandPackageOptions
            {
                ...
                ProjectDirectory = @"../../../NugetLicense.Toolkit.Tests.csproj",
                Timeout = 10
            });

            var packages = await methods.GetPackages();
            packages.Should().ContainKey("../../../NugetLicense.Toolkit.Tests.csproj");
        ...
        {
            var methods = new Methods(new CommandPackageOptions { ProjectDirectory = _projectPath, IgnoreSslCertificateErrors = true });
